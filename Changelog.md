...
### 改进
- **配置同步改为三方合并**（设计见 `docs/plans/sync-merge-rules-design.md`）：本机保存上一次同步结束时与云端一致的内容作为基线，同步时把本机和云端分别与它比较，从而分得清「谁改了什么」，不再是「同一个书签一律以云端为准」。
  - 本机的新增、修改、删除都会保留并传到其他设备；其他设备删除的书签也会在本机删除，已保存的密码随书签一起删除。
  - 两台设备分别改了不同的书签、不同的设置项、不同书签的密码，双方的修改都保留。改了同一项且结果不同才算冲突：以云端为准，同步结果会点名（`1 conflict — cloud copy kept: …`）。一边修改、另一边删除时保留修改。
  - 「最近使用时间」不算修改，两端取较新的一个。
  - **批量删除保护**：一次同步要删除 9 个及以上书签时（无论是本机删除要传到云端，还是云端删除要传到本机）先暂缓，其余改动照常同步；手动「立即同步」会弹窗确认后执行，自动同步不会执行。本机书签文件丢失（列表突然变空）时从云端恢复，而不是把云端清空。
  - 升级后的第一次同步仍按旧规则合并（并集、云端胜、不删除），从第二次起生效。**请尽快升级所有设备**：旧版本客户端不认识新的合并规则，仍会把已删除的书签加回云端；新版本遇到旧版本上传的副本时也只按旧规则合并，不会据此删除任何书签。全新设备上的「仅上传」同理，不会让其他设备删书签。
  - 同步过程中如果被另一台设备抢先上传，会自动重新合并一次，不再直接报错。
  - **快捷按钮、输出规则、书签文件夹按条目合并**：两台设备都改过同一个列表时不再是「整份以云端为准」——这台设备新增的快捷按钮和那台设备新增的输出规则都会保留，删除同样会传播；只有同一个条目两边改得不一样才算冲突，并在结果里点名。只有一边改过时，那一边的列表原样生效（包括排序）；新设备第一次同步时保留本机已有的条目。
- **同步结果改为本地化文案**：对话框、提示条里的同步结果不再是英文拼接，例如「双向同步完成。拉取：新增 1 个书签，更新 2 个，删除 1 个。上传：28 个书签。」冲突会写明是书签、凭据还是哪个设置项。
- **Live Sync 面板会留下需要处理的事项**：自动同步没有对话框可以提示，冲突与被暂缓的删除现在会留在「配置同步」一行里（并亮起提醒），直到下一次同步。
- **SSH 私钥一律从本地文件选择，不再粘贴**：书签编辑器的私钥、以及新建会话与书签编辑器里跳板机的私钥，由粘贴文本框改为「浏览…」选择本地文件，与新建会话的主私钥保持一致；界面只显示文件名，已随书签保存的私钥显示为「已保存的私钥」，不再把私钥内容显示出来。保存的仍是私钥内容（加密存入凭据库），因此配置同步到其他设备后照常可用。
  - 选错文件会当场提示，而不是等到连接时才报解析错误：选到 `.pub` 公钥会提示改选同名私钥；超过 64 KB 的文件不读取；不是 OpenSSH / PEM / PuTTY `.ppk` 私钥的文件会提示不支持。选择失败不会冲掉已经选好的私钥。

### 修复
- **配置同步不再回滚本地修改**：此前「立即同步」与自动同步总是先用云端副本覆盖本机再上传，改过的书签、已保存的密码、同步设置会在下一次同步时被还原，删除的书签会被重新加回来——包括书签变更 30 秒后自动触发的那一次。
...
- `SyncResult` 新增 `outcome`（结果类型）与结构化的 `conflicts`（`kind` / `name` / `item`），前端用 `describeSyncResult` 统一生成文案；`message` 仍是英文，仅用于日志。
- 同步基线存于本机 `sync_base.enc`（设备密钥加密）；凭据基线保存的是主密码封装的信封本身，不落任何可被设备密钥解开的凭据或其摘要。
- `rest-v2` 载荷新增可选字段 `lineage`（血缘）：每次上传继承所合并副本的血缘，首次上传、旧格式迁移、以及不是从当前云端副本出发的「仅上传」会开启新血缘；血缘不同的副本只做并集合并。服务端只校验 `schema`，无需改动。
